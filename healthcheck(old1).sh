...
# status가 failed이고 exit 값이 0이 아닌 경우 로그 파일의 exit 값을 리턴
...
if [[ "$status" == "running" && -n "$start_time" ]]; then
    ...
    if [ "$time_diff" -gt "$TIME_LIMIT" ]; then
        ...
        log_message "Info: Process completed within time limit. Exiting with code 0."
...
log_message "Error: Unknown error occurred. Exiting with code 255."
exit 255
